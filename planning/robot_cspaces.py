...
class CSpaceObstacleSolver:
        def __init__(self,space,milestones = (0,0),initial_points = 4000,steps = 100,max_iters = 3000):
            self.space = space
            #PRM planner
            MotionPlan.setOptions(type="prm",knn=40,connectionThreshold=0.5,ignoreConnectedComponents = True)
            self.optimizingPlanner = False
            # we first plan a bit without goals just to have a good skeleton
            self.initial_points = initial_points
            self.steps = steps
            self.orig_steps = steps
            self.milestones = milestones
            self.times = 0
        ...
        def compute_real_pairwise_distances(self,G_list):
            ...
            for i in tqdm(range(self.milestones.shape[0])):
                for j in range(i,self.milestones.shape[0]):
                    if(i!=j):
                        if(self.space.distance(self.milestones[i],self.milestones[j]) < 1):
                            if(self.space.visible(self.milestones[i],self.milestones[j])):
                                G_list[1].append((i,j))
            G.add_nodes_from(range(len(G_list[0])))
            G.add_edges_from(G_list[1])
            ...
            distances_array = np.zeros((self.milestones.shape[0],self.milestones.shape[0]))
            for i in tqdm(range(self.milestones.shape[0])):
                distances_dict = dict(nx.algorithms.shortest_path_length(G,source = i, weight = 'weight'))
                for j in range(i,self.milestones.shape[0]):
                    if(j not in distances_dict):
                        distances_array[i,j] = np.inf
                    else:
                        distances_array[i,j] = distances_dict[j]
        ...
        def get_adjacency_matrix_from_milestones(self):
            ...
            while(self.connected == False and self.return_partial == False):
                ...
                if(int(self.planner.getStats()['numIters']) >= self.max_iters):
                    # raise UnreachablePointsError('There are points in the planner that are not feasible after 3000 samples!')
                    print('could not connect all milestones. Returning partial prm mapping.')
                    self.return_partial = True
                G = nx.Graph()
                ...
                if(self.connected_list == self.total_milestones):
                    self.connected = True
                if(self.space.fraction < 0.3):
                    if(self.steps == self.orig_steps):
                        self.steps = max(int(self.steps//5),5)
